...
/* レジスタアドレスの算出 */
#define PC16550HAL_REG_ADDR(self, offset)	((volatile unsigned char *)(self)->pRegBase + ((self)->iRegStep * (offset)))
...
/** コンストラクタ */
void Pc16550Hal_Create(C_PC16550HAL *self, void *pRegAddr, int iRegStep, long lBaseClock)
{
	self->pRegBase   = pRegAddr;
	self->iRegStep   = iRegStep;
	self->lBaseClock = lBaseClock;
}
...
/* %jp{BPS設定} */
void Pc16550Hal_SetSpeed(C_PC16550HAL *self, long lSpeed)
{
	unsigned short uhDiv;

	uhDiv = (unsigned short)((self->lBaseClock / 16) / lSpeed);
