...
HANDLE XxxxDrv_Create(void *pRegBase, int iIntNum, unsigned long ulBaseClock, int iBufSize)
{
	C_XXXXDRV *self;
	...
	if ( XxxxDrv_Constructor(self, NULL, pRegBase, iIntNum, ulBaseClock, iBufSize) != FILE_ERR_OK )
	{
		SysMem_Free(self);
		return HANDLE_NULL;
	}
